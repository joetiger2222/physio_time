...
@Component({
  selector: 'app-viewsuspendedtimes',
  templateUrl: './viewsuspendedtimes.component.html',
  styleUrls: ['./viewsuspendedtimes.component.css']
})

export class ViewsuspendedtimesComponent {
  doctorId: string = '';
  ...
  deleteAppointment() {
    this.http
      .delete(
        `https://physiotime5-001-site1.anytempurl.com/api/Appointments/appointmentId?appointmentId=${this.appointmentId}`
      )
      .subscribe({
        next: (res) => {
          this.getReservedAppointements();
