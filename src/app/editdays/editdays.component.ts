...
constructor(private http: HttpClient,private route:ActivatedRoute) { }
  ...
  getDoctorDays(appts:Appointment[]){
    this.http.get<DoctorDay[]>('http://physiotime778-001-site1.ftempurl.com/api/Doctors/DoctorDays/'+this.doctorId).subscribe({
      next:res=>{
  ...
  getReservedAppointements() {
    ...
    this.http
      .get<Appointment[]>(
        `http://physiotime778-001-site1.ftempurl.com/api/Appointments/ReservedAppointments/${this.doctorId}`
      )
      .subscribe({
        next: (res) => {
  ...
  DeleteDay(){
    this.isLoading=true
    this.http.delete(`http://physiotime778-001-site1.ftempurl.com/api/Doctors/DoctorDay/${this.choosenDayId}`).subscribe({
      next:res=>{
        this.isLoading=false
        this.getReservedAppointements();
  ...
  editDay(){
    ...
    this.http.put(`http://physiotime778-001-site1.ftempurl.com/api/Doctors/DoctorDays/${this.choosenDayId}`,body).subscribe({
      next:res=>{
        this.isLoading=false
        this.getReservedAppointements();
        this.CloseEditModal();
