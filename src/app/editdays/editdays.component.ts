...
constructor(private http: HttpClient,private route:ActivatedRoute) { }
  ...
  getDoctorDays(appts:Appointment[]){
    this.http.get<DoctorDay[]>('https://joetiger123456-001-site1.atempurl.com/api/Doctors/DoctorDays/'+this.doctorId).subscribe({
      next:res=>{
  ...
  DeleteDay(){
    this.isLoading=true
    this.http.delete(`https://joetiger123456-001-site1.atempurl.com/api/Doctors/DoctorDay/${this.choosenDayId}`).subscribe({
      next:res=>{
        this.isLoading=false
        this.getReservedAppointements();
  ...
  editDay(){
    ...
    this.http.put(`https://joetiger123456-001-site1.atempurl.com/api/Doctors/DoctorDays/${this.choosenDayId}`,body).subscribe({
      next:res=>{
        this.isLoading=false
        this.getReservedAppointements();
        this.CloseEditModal();
