...
constructor(private http: HttpClient,private route:ActivatedRoute) { }
  ...
  getDoctorDays(appts:Appointment[]){
    this.http.get<DoctorDay[]>('https://physiotime2-001-site1.gtempurl.com/api/Doctors/DoctorDays/'+this.doctorId).subscribe({
      next:res=>{
  ...
  DeleteDay(){
    this.isLoading=true
    this.http.delete(`https://physiotime2-001-site1.gtempurl.com/api/Doctors/DoctorDay/${this.choosenDayId}`).subscribe({
      next:res=>{
        this.isLoading=false
        this.getReservedAppointements();
  ...
  editDay(){
    ...
    this.http.put(`https://physiotime2-001-site1.gtempurl.com/api/Doctors/DoctorDays/${this.choosenDayId}`,body).subscribe({
      next:res=>{
        this.isLoading=false
        this.getReservedAppointements();
        this.CloseEditModal();
