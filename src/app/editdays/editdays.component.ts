...
constructor(private http: HttpClient,private route:ActivatedRoute) { }
  ...
  getDoctorDays(appts:Appointment[]){
    this.http.get<DoctorDay[]>('https://physiotime5-001-site1.anytempurl.com/api/Doctors/DoctorDays/'+this.doctorId).subscribe({
      next:res=>{
  ...
  getReservedAppointements() {
    ...
    this.http
      .get<Appointment[]>(
        `https://physiotime5-001-site1.anytempurl.com/api/Appointments/ReservedAppointments/${this.doctorId}`
      )
      .subscribe({
        next: (res) => {
  ...
  DeleteDay(){
    this.isLoading=true
    this.http.delete(`https://physiotime5-001-site1.anytempurl.com/api/Doctors/DoctorDay/${this.choosenDayId}`).subscribe({
      next:res=>{
        this.isLoading=false
        this.getReservedAppointements();
  ...
  editDay(){
    ...
    this.http.put(`https://physiotime5-001-site1.anytempurl.com/api/Doctors/DoctorDays/${this.choosenDayId}`,body).subscribe({
      next:res=>{
        this.isLoading=false
        this.getReservedAppointements();
        this.CloseEditModal();
